...
// === AQUA SCOPE: đã cắt OTA / mDNS / NTP / captive portal ====================
// Bản gốc easytarget nạp thêm ArduinoOTA, ESPmDNS, DNSServer, WiFiUdp, time.h.
// Không cái nào đụng vào đường camera, nhưng mỗi cái đều chạy task nền ăn heap
// — thứ mà ESP32-CAM vốn đã thiếu khi chụp UXGA. Trạm này nạp qua USB, dùng IP
// trực tiếp, và laptop mới là nơi đóng dấu thời gian, nên cả 4 đều thừa.
//
// Các biến cờ (otaEnabled / haveTime / captivePortal) được GIỮ LẠI và đặt cứng
// false: app_httpd.cpp tham chiếu tới chúng, giữ vậy thì file đó không phải sửa
// một dòng nào — đúng tinh thần "copy y hệt để tránh lỗi".
// ============================================================================
...
// Captive portal đã cắt; giữ cờ cho app_httpd.cpp tham chiếu.
...
// OTA và NTP đã cắt. Giữ biến vì app_httpd.cpp in chúng ra trang trạng thái.
...
void setLamp(int newVal) {
    ...
    if (newVal != -1) {
        ...
        ledcWrite(LAMP_PIN, brightness);  // core 3.x: ledcWrite theo PIN, không theo channel
        Serial.print("Lamp: ");
        Serial.print(newVal);
        Serial.print("%, pwm = ");
...
// NTP đã cắt. Hàm giữ lại làm no-op vì app_httpd.cpp khai báo extern và gọi nó
// (chỉ khi haveTime == true, tức là không bao giờ).
...
void StartCamera() {
    ...
    if (err != ESP_OK) {
        ...
        // Start a 60 second watchdog timer (core 3.x API: reconfigure the already-running TWDT)
        esp_task_wdt_config_t twdt_cfg = { .timeout_ms = 60000, .idle_core_mask = 0, .trigger_panic = true };
        ...
        // === AQUA SCOPE TEST OVERRIDE =====================================
        // Ep bien `sensorPID` (dung cho logic cua Aqua Scope: chon giao dien
        // + preset ben duoi) thanh OV3660, BAT KE chip that tra ve gi.
        //
        // AN TOAN vi day CHI la bien logic tang ung dung. Duong giao tiep
        // thuc voi sensor (cac ham s->set_*) da duoc esp32-camera gan dung
        // driver theo PID THAT ma no doc qua SCCB ngay trong esp_camera_init()
        // o tren - ep bien nay KHONG doi lai duong do.
        //
        // Dung khi: test giao dien index_ov3660.h ma khong can cam board that
        // co OV3660. KHONG dung khi chay that voi phan cung that - de vay se
        // ap preset OV3660 (vflip/brightness/saturation) len chip OV2640 that,
        // anh se bi sai chieu/sang mau khong dung y muon.
        //
        // Bo comment dong duoi de bat:
        // #define AQUA_FORCE_OV3660_UI
        #if defined(AQUA_FORCE_OV3660_UI)
            Serial.println("[AQUA TEST OVERRIDE] Ep sensorPID = OV3660 (bo qua nhan dien that)");
...
void WifiSetup() {
    ...
    if (accesspoint && (WiFi.status() != WL_CONNECTED)) {
        ...
        // (Captive portal đã cắt — nối thẳng bằng IP.)
    }
}
...
void setup() {
    ...
    */
    ...
    // OTA / mDNS / NTP đã cắt (xem ghi chú ở đầu file).
    ...
    if (lampVal != -1) {
        #if defined(LAMP_PIN)
            ledcAttach(LAMP_PIN, pwmfreq, pwmresolution);    // core 3.x: attach pin + configure PWM in one call
