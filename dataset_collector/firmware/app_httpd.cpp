...
static esp_err_t cmd_handler(httpd_req_t *req){
    ...
    if(!strcmp(variable, "framesize")) {
        ...
        // === Chế độ BUỒNG TỐI cho Aqua Scope (ảnh silhouette backlit) ===
        // Gộp cả bộ thông số phơi sáng thủ công vào 1 lệnh. Thứ tự quan trọng:
        // phải TẮT AEC/AGC trước rồi mới đặt giá trị exposure/gain thủ công.
        s->set_gain_ctrl(s, 0);                   // AGC OFF  (nếu bật, sensor tự kéo sáng -> cháy nền trắng)
        ...
        s->set_aec_value(s, 200);                 // phơi sáng thủ công thấp: nền sáng đều, không cháy trắng (tinh chỉnh bằng thanh Exposure)
        s->set_brightness(s, 0);                  // độ sáng trung tính
        s->set_contrast(s, 2);                    // TĂNG tương phản -> hạt tối nổi rõ trên nền sáng
        s->set_saturation(s, -2);                 // giảm bão hòa (gần grayscale, bớt nhiễu màu ở rìa hạt)
        s->set_whitebal(s, 1);                    // cân bằng trắng cho nền LED trắng
        s->set_awb_gain(s, 1);
        s->set_lenc(s, 1);                        // bù sáng rìa ống kính -> nền đều hơn
        s->set_bpc(s, 1);                         // bù điểm ảnh đen lỗi
        s->set_wpc(s, 1);                         // bù điểm ảnh trắng lỗi
        s->set_raw_gma(s, 1);                     // gamma -> giữ chi tiết vùng sáng
        s->set_dcw(s, 1);
        s->set_special_effect(s, 0);              // không hiệu ứng (giữ màu; đổi 2 nếu muốn grayscale hẳn)
        Serial.println("Dark mode preset applied (backlit silhouette)");
        res = 0;
    }
        ...
        for (int i = 0; i < 20; i++) {  // ~3 giây nhấp nháy phản hồi trước khi khởi động lại
          flashLED(50);
          delay(100);
          Serial.print('.');
        }
        esp_restart();  // core 3.x: khởi động lại sạch, không cần watchdog panic
    }
    else {
        res = -1;
    }
