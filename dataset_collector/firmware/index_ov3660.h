...
<html>
  ...
  <body>
    <section class="main">
      <div id="logo">
        ...
        <button id="darkmode" style="float:left;" title="Che do buong toi: set thong so cho anh silhouette backlit">Dark</button>
        <button id="dataset-shot" style="float:left;" title="Chup 1 anh va gui ve PC de luu vao dataset">&#128190; Dataset</button>
        <span id="dataset-status" style="float:left;margin-left:8px;line-height:2em;font-size:0.9em;">&nbsp;</span>
      ...
      <div id="content">
        <div class="hidden" id="sidebar">
          <input type="checkbox" id="nav-toggle-cb" checked="checked">
            <nav id="menu">
              <div class="input-group" id="dataset-host-group" title="Dia chi may tinh dang chay: python collect_dataset.py --serve&#013;Vi du: 192.168.1.20:8765&#013;Dia chi nay duoc nho lai trong trinh duyet.">
                <label for="dataset_host">Dataset PC</label>
                <input type="text" id="dataset_host" size="16" placeholder="192.168.1.20:8765">
              </div>
              <div class="input-group" id="dataset-burst-group" title="So anh chup lien tiep moi lan bam nut Dataset">
                <label for="dataset_burst">Burst</label>
                <input type="number" id="dataset_burst" min="1" max="500" value="1" style="width:5em;">
              </div>
  ...
  document.addEventListener('DOMContentLoaded', function (event) {
    ...
    const datasetButton = document.getElementById('dataset-shot')
    const datasetStatus = document.getElementById('dataset-status')
    const datasetHost = document.getElementById('dataset_host')
    const datasetBurst = document.getElementById('dataset_burst')
    ...
    // === THU DATASET =========================================================
    // Trinh duyet lam cau noi: lay /capture tu ESP32 roi POST sang script
    // `collect_dataset.py --serve` chay tren PC. Firmware KHONG can biet dia chi
    // PC, khong can HTTP client - moi thu nam trong doan JS nay.
    let datasetSent = 0
    let datasetBusy = false

    const setDatasetStatus = (msg, isError) => {
      datasetStatus.textContent = msg
      datasetStatus.style.color = isError ? '#ff4d4d' : '#a0d0a0'
    }

    datasetHost.value = localStorage.getItem('aquaDatasetHost') || ''
    datasetHost.onchange = () => {
      localStorage.setItem('aquaDatasetHost', datasetHost.value.trim())
    }

    const sendOneToDataset = async (host) => {
      // Cache-bust bat buoc: khong co no trinh duyet tra lai anh cu trong cache
      // va dataset se day anh trung nhau ma nhin log van tuong la dang chup moi.
      const shot = await fetch(`${baseHost}/capture?_cb=${Date.now()}`)
      if (!shot.ok) throw new Error(`ESP32 tra ve HTTP ${shot.status}`)
      const blob = await shot.blob()
      if (blob.size === 0) throw new Error('anh rong')

      let up
      try {
        up = await fetch(`http://${host}/upload`, {
          method: 'POST',
          headers: { 'Content-Type': 'image/jpeg' },
          body: blob
        })
      } catch (e) {
        // Trinh duyet chi noi "Failed to fetch" - vo nghia voi nguoi dung.
        // Doi thanh nguyen nhan that su va cach sua.
        throw new Error(`khong ket noi duoc ${host} - da chay "collect_dataset.py --serve" chua?`)
      }
      if (!up.ok) {
        const detail = await up.text().catch(() => '')
        throw new Error(`PC tu choi (HTTP ${up.status}) ${detail}`)
      }
      return blob.size
    }

    datasetButton.onclick = async () => {
      if (datasetBusy) return           // chan bam lien tuc lam chong cheo request
      const host = datasetHost.value.trim()
      if (!host) {
        setDatasetStatus('Chua nhap "Dataset PC" trong Settings', true)
        return
      }
      const total = Math.max(1, Math.min(500, parseInt(datasetBurst.value) || 1))
      ...
      try {
        for (let i = 1; i <= total; i++) {
          setDatasetStatus(`Dang chup ${i}/${total}...`, false)
          await sendOneToDataset(host)
          done++
          datasetSent++
        }
        setDatasetStatus(`OK - da gui ${datasetSent} anh`, false)
      } catch (err) {
        // Bao ro da gui duoc bao nhieu truoc khi hong, de biet co phai chup lai tu dau khong
        setDatasetStatus(`Loi sau ${done}/${total}: ${err.message}`, true)
    ...
    // Che do buong toi: gui preset silhouette backlit roi nap lai cac thanh truot
    darkButton.onclick = () => {
      fetch(`${baseHost}/control?var=darkmode&val=1`)
        .then(() => fetch(`${baseHost}/status`))
        ...
        .then(state => {
          ...
          console.log('Dark mode applied, controls refreshed')
        })
    }
