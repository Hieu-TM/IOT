...
Không viết sẵn code cho những thứ này. Chỗ để dành duy nhất được chấp nhận: `/status`
...
- `GET http://<host>/status` một lần lúc khởi tạo → giữ lại để nhét vào metadata.
...
- `sample_code` sinh dạng `S{yyyyMMdd}-{HHmmss}-{mmm}` từ thời điểm chụp, đi qua
  `naming.sample_code_from_filename` để đảm bảo khớp pattern của ingest contract.
...
`config.missing_for()` phải biết kiểm tra mục này khi chạy `--from-board`.
...
- `/status` không gọi được → lỗi rõ ràng, dừng
- `/status` trả JSON thiếu khóa → không sập, báo thiếu
...
2. `GET /status` → JSON hợp lệ, `device_id` khớp MAC, `psram: true`.
3. Web UI canh sáng → nền xám đều, hạt là bóng đen.
4. `var=save` → rút điện → cắm lại → `/status` báo `prefs_saved: true` và đúng thông số.
5. Tắt router 30 giây rồi bật lại → board tự nối lại, `/status` phản hồi, không cần reset.
