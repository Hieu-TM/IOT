// ============================================================================
// tube_body_002.scad — Thân ống quang học (đoạn TRÊN của vỏ liền — G2)
// ----------------------------------------------------------------------------
// KHÁC _001: BỎ VÒNG ĐỠ KHAY z=−8..−6.5 (ID40.5). Vòng này CHẶN ĐƯỜNG LUỒN:
// khay OD44 lắp từ ĐÁY không thể chui qua lỗ ID40.5 — mâu thuẫn lắp ráp,
// phát hiện khi in thật 2026-07-08. Khay nay được đỡ bằng 2 NÚT BỊT KHE KIÊM
// CỘT ĐỠ chữ L (slot_plug — light_box_002): luồn vào 2 khe dọc ±X SAU khi đưa
// khay lên, gờ ngang đỡ đáy váy khay tại z=−6.5, chân đứng cùng mặt bàn với đế.
// (Vỏ đã in theo _001 có thể TẬN DỤNG: cắt/gọt bỏ vòng đỡ cũ + dũa nóc khe +0.5.)
//
// Dựng TRỰC TIẾP trong hệ toạ độ chung (trục Z = trục quang, z=0 = sàn khay).
// Phạm vi đoạn này: z = −8 (ranh giới với light_box) .. 46 (mặt bích đỉnh).
...
//  - Vòng chặn khay z=12..14 (ID40.5): cữ TRÊN của khay (khay luồn từ dưới,
//    tì lên cột đỡ nút bịt); kiêm CHẮN SÁNG khe hở khay↔lòng ống (buồng tối).
//  - 2 KHE DỌC ±X (RA 12 / VÀO 24, lên tới z=+7.5): đường thò ngạnh ống nước
//    của khay khi luồn từ đáy + đường dẫn ống ra ngoài; nóc khe cao hơn đỉnh
//    ngạnh 0.5 để ngạnh không chạm nóc trước khi khay tì lên cột đỡ.
//    (Ốc vách LED nằm ±Y nên không đụng.)
// ============================================================================
...
module tube_body() {
    ...
    // (Vòng đỡ khay z=−8..−6.5 của _001 ĐÃ BỎ — xem chú thích đầu file.)
}
